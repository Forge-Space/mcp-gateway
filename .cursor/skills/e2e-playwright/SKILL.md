...
- Verifying login, dashboard, or feature flows in the browser
- Debugging UI or interaction issues with live browser tools

## Layout (adapt to project)

- E2E tests: typically under `tests/`, `e2e/`, or `packages/frontend/tests/`
- Playwright config: `playwright.config.ts` at repo or frontend package root

## Running E2E

- Start app: `npm run dev` or `npm run dev:frontend` (or use Docker)
- Run Playwright: `npx playwright test` (from package or root, per project)
...
- Test critical user flows; avoid testing implementation details.
