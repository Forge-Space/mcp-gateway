...
- **uiforge-mcp server v0.4.1** – Updated local translate service (port 8026) providing 12 AI-driven UI generation tools with full framework parity: `scaffold_full_application` (React/Next.js/Vue/Angular/HTML/Svelte + Tailwind + Shadcn/ui), `generate_ui_component` (style-aware components with audit), `generate_prototype` (interactive HTML prototypes with navigation), `generate_design_image` (SVG/PNG mockups via satori + resvg), `fetch_design_inspiration` (extract colors/typography from URLs), `analyze_design_references` (pattern detection from multiple sources), `figma_context_parser` (read Figma nodes → Tailwind mapping), `figma_push_variables` (write design tokens to Figma Variables API), `image_to_component` (screenshot/mockup to code conversion), `generate_page_template` (pre-built templates), `refine_component` (iterative improvements), `audit_accessibility` (WCAG 2.1 AA/AAA compliance). Plus 1 MCP resource: `application://current-styles` (session-scoped design context). Set `FIGMA_ACCESS_TOKEN` in `.env` for Figma tools. Added to `cursor-default` virtual server. Built from [uiforge-mcp](https://github.com/LucasSantana-Dev/uiforge-mcp) v0.4.1 via `Dockerfile.uiforge`.
