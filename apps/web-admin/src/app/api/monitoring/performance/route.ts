...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${GATEWAY_URL}/monitoring/performance`, {
    ...
    console.error('Failed to fetch monitoring performance from gateway:', error);
    return NextResponse.json({ error: 'Failed to fetch monitoring performance' }, { status: 500 });
  }
}
