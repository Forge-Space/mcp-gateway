...
class TestDockerfiles:
    ...
    def test_dockerfile_tool_router_has_from_statement(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_uses_python_base(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_specifies_python_version(self, dockerfile_tool_router: Path) -> None:
        ...
        assert re.search(python_version_pattern, content, re.IGNORECASE), "Should specify Python version"

    def test_dockerfile_tool_router_uses_minimal_image(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_copies_requirements(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_installs_dependencies(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_uses_non_root_user(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_creates_app_user(self, dockerfile_tool_router: Path) -> None:
        """Test that tool-router Dockerfile creates app user."""
        with open(dockerfile_tool_router) as f:
            content = f.read()

        # Should create user (adduser or useradd or addgroup)
        creates_user = "adduser" in content.lower() or "useradd" in content.lower() or "addgroup" in content.lower()
        assert creates_user, "Should create non-root user"

    def test_dockerfile_tool_router_has_healthcheck(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_sets_python_env_vars(self, dockerfile_tool_router: Path) -> None:
        """Test that tool-router Dockerfile sets Python environment variables."""
        with open(dockerfile_tool_router) as f:
            content = f.read()

        # Should set PYTHONUNBUFFERED or PYTHONDONTWRITEBYTECODE
        has_python_env = "PYTHONUNBUFFERED" in content or "PYTHONDONTWRITEBYTECODE" in content
        assert has_python_env, "Should set Python environment variables"

    def test_dockerfile_tool_router_cleans_cache(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_dribbble_has_from_statement(self, dockerfile_dribbble: Path) -> None:
        """Test that dribbble Dockerfile has FROM statement."""
        with open(dockerfile_dribbble) as f:
            content = f.read()

        assert re.search(r"^FROM\s+", content, re.MULTILINE), "Should have FROM statement"

    def test_dockerfile_dribbble_uses_python_base(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_dribbble_installs_playwright(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_dribbble_installs_chromium(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_dribbble_uses_non_root_user(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_dribbble_has_healthcheck(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_dribbble_sets_python_env_vars(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_dribbble_sets_playwright_env(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_tool_router_multi_line_commands_use_continuation(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_copies_application_code(self, dockerfile_tool_router: Path) -> None:
    ...
    def test_dockerfile_tool_router_runs_simple_server(self, dockerfile_tool_router: Path) -> None:
        """Test that tool-router Dockerfile runs the tool router module."""
        with open(dockerfile_tool_router) as f:
            content = f.read()

        # Should run tool_router module
        assert '"-m", "tool_router"' in content, "Should run tool_router module"
    ...
    def test_dockerfile_dribbble_copies_application_code(self, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_security_updates(self, dockerfile_tool_router: Path, dockerfile_dribbble: Path) -> None:
        ...
        for dockerfile in [dockerfile_tool_router, dockerfile_dribbble]:
            ...
            has_updates = "apt-get update" in content.lower() or "apk update" in content.lower()
            # This is good practice but not always required

    def test_dockerfile_layer_optimization(self, dockerfile_tool_router: Path, dockerfile_dribbble: Path) -> None:
    ...
    def test_dockerfile_tool_router_no_secrets_hardcoded(self, dockerfile_tool_router: Path) -> None:
        ...
        for pattern in dangerous_patterns:
            ...
            real_secrets = [
                m
                for m in matches
                if not any(placeholder in m.lower() for placeholder in ["your_", "example", "xxx", "placeholder"])
            ]
            assert not real_secrets, f"Should not contain hardcoded secrets: {real_secrets}"

    def test_dockerfile_dribbble_no_secrets_hardcoded(self, dockerfile_dribbble: Path) -> None:
