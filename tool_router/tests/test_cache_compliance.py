...
from cache.types import (
    DataClassification, ComplianceStandard, ConsentRecord, AuditEntry,
    CacheEntryMetadata, SecurityMetrics, ComplianceError, AuditError
)
from cache.compliance import (
    ComplianceManager, GDPRComplianceHandler, ComplianceReporter,
    ComplianceStatus, DataSubjectRequestType, ComplianceAssessment,
...
class TestGDPRComplianceHandler:
    ...
    def test_consent_validation_positive(self):
        ...
        # Check valid consent
        has_consent = self.gdpr_handler.check_consent(
            subject_id, 'email', 'newsletter'
        )
        assert has_consent is True
        
        # Check invalid data type
        has_consent = self.gdpr_handler.check_consent(
            subject_id, 'phone', 'newsletter'
        )
        assert has_consent is False
        
        # Check invalid purpose
        has_consent = self.gdpr_handler.check_consent(
            subject_id, 'email', 'marketing'
        )
        assert has_consent is False
    ...
    def test_consent_withdrawal(self):
        ...
        assert self.gdpr_handler.check_consent(
            subject_id, 'personal_data', 'analytics'
        ) is True
    ...
    def test_consent_expiration_handling(self):
        ...
        assert self.gdpr_handler.check_consent(
            subject_id, 'test_data', 'test_purpose'
        ) is False
...
class TestComplianceManager:
    ...
    def test_consent_management_workflow(self):
        ...
        has_consent = self.compliance_manager.check_consent(
            subject_id, 'phone', 'personalization'  # Different data type
        )
        assert has_consent is False
...
class TestComplianceIntegration:
    ...
    def test_consent_driven_data_processing_workflow(self):
        ...
        consent_id = self.compliance_manager.record_consent(user_id, {
