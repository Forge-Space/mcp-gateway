...
class TestFeedbackStoreTaskTypeBoost:
    ...
    def test_get_task_type_boost_range(self, tmp_path: Path) -> None:
        store = FeedbackStore(str(tmp_path / "fb.json"))
        # "read file" classifies as file_operations
        store.record("read file", "tool", success=True)
        boost_high = store.get_task_type_boost("tool", "file_operations")
        store.record("read file", "tool2", success=False)
        boost_low = store.get_task_type_boost("tool2", "file_operations")
...
class TestFeedbackStoreLearningInsights:
    ...
    def test_get_learning_insights_empty(self, tmp_path: Path) -> None:
        ...
        assert insights["pattern"] is None
...
class TestFeedbackStoreAdvancedMethods:
    ...
    def test_recent_success_rate_calculation(self, tmp_path: Path) -> None:
        ...
        stats = store.get_stats("tool")
        assert stats is not None
        # Last 50 entries: entries 10-59. Successes: 10-39 = 30.
    ...
    def test_task_pattern_learning(self, tmp_path: Path) -> None:
        ...
        assert pattern.preferred_tools["file_tool"] > 0.5
...
class TestFeedbackStoreBusinessLogic:
    """Test FeedbackStore business logic and learning algorithms."""
    ...
    def test_confidence_tracking_affects_boost(self, tmp_path: Path) -> None:
        store = FeedbackStore(str(tmp_path / "fb.json"))
        # Need 3+ entries per tool for boost threshold
        for _ in range(3):
            store.record(
                "task",
                "high_conf_tool",
    ...
    def test_recent_performance_affects_boost(self, tmp_path: Path) -> None:
        ...
        # Mixed history: 50% success, but recent has confidence
        assert boost >= 0.5
    ...
    def test_pattern_learning_for_task_types(self, tmp_path: Path) -> None:
        ...
        assert pattern.total_occurrences >= 8
    ...
    def test_entity_extraction_and_learning(self, tmp_path: Path) -> None:
        store = FeedbackStore(str(tmp_path / "fb.json"))
        # "read /path..." -> file_operations
        store.record("read /path/to/file.py", "processor", success=True)
        store.record("handle 'important data'", "handler", success=True)
        # "fetch https..." -> network_operations
        store.record(
        ...
        assert "file_operations" in store._patterns
        file_pattern = store._patterns["file_operations"]
        assert "/path/to/file.py" in file_pattern.common_entities
    ...
    def test_similar_task_tools_uses_learning(self, tmp_path: Path) -> None:
        ...
        assert "doc_search" in similar
        assert "error_search" not in similar
