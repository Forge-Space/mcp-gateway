...
class TestHTTPGatewayClient:
    ...
    def test_headers(self, config):
        ...
        assert headers["Authorization"] == f"Bearer {config.jwt}"
        assert "Content-Type" in headers
        assert headers["Content-Type"] == "application/json"

    def test_make_request_success(self, config):
    ...
    def test_make_request_with_data(self, config):
    ...
    def test_make_request_http_error_4xx(self, config):
        ...
        with patch("urllib.request.urlopen") as mock_urlopen:
            ...
            mock_urlopen.side_effect = urllib.error.HTTPError(
                url="https://api.example.com/test", code=404, msg="Not Found", hdrs={}, fp=io.BytesIO(b"Not Found")
            )

            with pytest.raises(ValueError, match="Gateway HTTP error 404"):
                client._make_request("https://api.example.com/test")

    def test_make_request_http_error_5xx_with_retry(self, config):
        ...
        with patch("urllib.request.urlopen") as mock_urlopen, patch("time.sleep"):
            ...
            mock_urlopen.side_effect = [
                ...
                Mock(__enter__=Mock(return_value=success_response), __exit__=Mock(return_value=None)),
            ]
    ...
    def test_make_request_max_retries_exceeded(self, config):
    ...
    def test_make_request_network_error_with_retry(self, config):
    ...
    def test_make_request_timeout_with_retry(self, config):
    ...
    def test_make_request_json_decode_error(self, config):
    ...
    def test_get_tools_success_list_response(self, config):
    ...
    def test_get_tools_success_dict_response(self, config):
    ...
    def test_get_tools_json_decode_error(self, config):
    ...
    def test_get_tools_connection_error(self, config):
        """Test getting tools with connection error."""
        client = HTTPGatewayClient(config)

        with patch.object(client, "_make_request") as mock_request:
            mock_request.side_effect = ConnectionError("Failed after 3 attempts. Last error: Network error")

            with pytest.raises(ValueError, match="Failed to fetch tools"):
                client.get_tools()

    def test_get_tools_value_error(self, config):
    ...
    def test_call_tool_success(self, config):
    ...
    def test_call_tool_success_no_content(self, config):
    ...
    def test_call_tool_with_error(self, config):
    ...
    def test_call_tool_request_error(self, config):
    ...
    def test_call_tool_connection_error(self, config):
...
class TestInfrastructureIntegration:
    ...
    def test_gateway_client_with_config_integration(self):
        ...
        assert client._retry_delay_seconds == 0.5
