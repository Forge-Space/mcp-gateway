"""Enhanced rate limiting and request throttling for AI agent requests with configurable caching."""
...
class EnhancedRateLimiter:
    ...
    def check_rate_limit(
        ...
        with self._lock:
            if cache_key in self._result_cache:
                self._cache_hits["rate_limit"] += 1
                cached_result = self._result_cache[cache_key]
                # Update reset time if needed
                cached_result.reset_time = current_time + 60
                return cached_result
            self._cache_misses["rate_limit"] += 1
        ...
        if self._is_penalized(identifier, current_time):
            ...
            # Cache penalty result for shorter duration
            with self._lock:
        ...
        if most_restrictive:
            with self._lock:
                self._result_cache[cache_key] = most_restrictive
    ...
    def _apply_adaptive_scaling(
        self, identifier: str, result: RateLimitResult, config: RateLimitConfig
    ) -> RateLimitResult:
    ...
    def _is_penalized(self, identifier: str, current_time: int) -> bool:
        """Check if identifier is currently penalized with caching."""
        # Check cache first
        with self._lock:
            if identifier in self._penalty_cache:
                penalty_end = self._penalty_cache[identifier]
                if current_time < penalty_end:
                    return True
                # Penalty expired, remove from cache
                del self._penalty_cache[identifier]
    ...
    def apply_penalty(self, identifier: str, duration: int) -> None:
        ...
        # Cache the penalty
        with self._lock:
            self._penalty_cache[identifier] = penalty_end
    ...
    def clear_penalties(self, identifier: str) -> None:
        ...
        # Remove from cache
        with self._lock:
            self._penalty_cache.pop(identifier, None)
    ...
    def get_usage_stats(self, identifier: str) -> dict[str, any]:
        """Get usage statistics for an identifier with caching."""
        current_time = int(time.time())
        ...
        with self._lock:
            ...
            self._cache_misses["usage_stats"] += 1
    ...
    def get_cache_metrics(self) -> dict[str, any]:
        ...
        return {
            ...
            "cache_sizes": {
                ...
                "usage_cache": len(self._usage_cache),
                "penalty_cache": len(self._penalty_cache),
            },
    ...
    def cleanup_expired_data(self) -> None:
        ...
        for ident in expired_penalties:
            ...
            with self._lock:
                self._penalty_cache.pop(ident, None)
