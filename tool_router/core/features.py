...
class FeatureFlags:
    ...
    config_file: Path = field(default_factory=lambda: Path(__file__).parent.parent.parent / "config" / "features.yaml")
        ...
        for feature in self.features.values():
            ...
            if feature.env_var:
                ...
                if env_value is not None:
                    ...
                    continue  # Primary takes precedence, skip backward compat

            # Check backward compatibility env var only if primary not set
    ...
    def get_features_by_category(self, category: str) -> dict[str, Feature]:
        ...
        Returns:
            Dictionary of feature name to Feature object
        """
        return {name: feature for name, feature in self.features.items() if feature.category == category}
