...
class UIStandardsCompliance:
    ...
    def _check_design_system_compliance(self, component_spec: ComponentSpec, design_system: DesignSystem) -> float:
        ...
        if design_system == DesignSystem.MATERIAL_DESIGN:
            return 0.9 if "material" in component_spec.styling else 0.7
        if design_system == DesignSystem.BOOTSTRAP:
...
class UISpecialist:
    ...
    def _get_required_accessibility_features(self, level: AccessibilityLevel) -> list[str]:
        ...
        if level == AccessibilityLevel.AAA:
            return [
                "color_contrast_7_1",
                "no_reflow",
                "text_spacing",
        ...
        return ["basic_keyboard_access", "alt_text_important", "semantic_structure"]
